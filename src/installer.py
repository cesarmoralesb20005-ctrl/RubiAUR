...
class RubiAURInstaller(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("RubiAUR Installer")
        ...
        # Definir las rutas clave del sistema
        ...
        # Comprobamos si la app ya está instalada
        self.check_status()
        ...
        title = QLabel("RubiAUR Setup")
        title.setFont(QFont("SF Pro Display", 28, QFont.Bold))
        title.setAlignment(Qt.AlignCenter)

        desc = QLabel("Install RubiAUR on your system to add a desktop entry,\nor uninstall it if you no longer need it.")
        ...
        # Configuración de Botones basada en el estado
        btn_row = QHBoxLayout()
        btn_row.setSpacing(15)

        self.btn_desinstalar = QPushButton("Uninstall")
        ...
        # Si NO está instalada, bloqueamos el botón de desinstalar
        if not self.is_installed:
            self.btn_desinstalar.setEnabled(False)

        # Cambiamos el texto dinámicamente: Install o Update
        action_text = "Update" if self.is_installed else "Install"
        self.btn_instalar = QPushButton(action_text)
    ...
    def check_status(self):
        """Verifica si el binario de RubiAUR ya existe en el sistema"""
        self.is_installed = os.path.exists(self.bin_destino)

    def ejecutar_instalacion(self):
        action_name = "Updating..." if self.is_installed else "Installing..."
        self.btn_instalar.setEnabled(False)
        self.btn_desinstalar.setEnabled(False)
        self.btn_instalar.setText(action_name)
        ...
        bin_origen = "RubiAUR-x86_64.AppImage" 
        ...
        try:
            ...
            if not os.path.exists(logo_origen):
                raise Exception(f"File not found: {logo_origen}")
...
Type=Application
Name=RubiAUR
Comment=Elegant AUR package manager
...
"""
            ...
            self.status_lbl.setStyleSheet("color: #34C759; font-size: 14px; font-weight: bold;")
            success_msg = "Update completed!" if self.is_installed else "Installation successful!"
            self.status_lbl.setText(success_msg)
            
            # Cambiar el botón para cerrar el instalador
            self.btn_instalar.setText("Close")
            self.btn_instalar.clicked.disconnect()
            self.btn_instalar.clicked.connect(self.close)
            self.btn_instalar.setEnabled(True)

        except Exception as e:
            self.status_lbl.setStyleSheet("color: #FF453A; font-size: 13px; font-weight: bold;")
            self.status_lbl.setText(str(e))
            self.btn_instalar.setText("Retry")
            self.btn_instalar.setEnabled(True)
    ...
    def ejecutar_desinstalacion(self):
        self.btn_instalar.setEnabled(False)
        self.btn_desinstalar.setEnabled(False)
        self.btn_desinstalar.setText("Uninstalling...")
        QApplication.processEvents()

        try:
            # Eliminar los archivos si existen
            if os.path.exists(self.bin_destino): os.remove(self.bin_destino)
            if os.path.exists(self.logo_destino): os.remove(self.logo_destino)
            if os.path.exists(self.desktop_destino): os.remove(self.desktop_destino)

            self.status_lbl.setStyleSheet("color: #FF453A; font-size: 14px; font-weight: bold;")
            self.status_lbl.setText("RubiAUR was successfully uninstalled!")
            
            self.btn_desinstalar.setText("Close")
            ...
            self.btn_desinstalar.setText("Retry")
            self.btn_instalar.setEnabled(True)
            self.btn_desinstalar.setEnabled(True)

if __name__ == "__main__":
    app = QApplication(sys.argv)
    window = RubiAURInstaller()
    window.show()
    sys.exit(app.exec())