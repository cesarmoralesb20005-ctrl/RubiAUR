...
from workers import (AutocompleteWorker, CheckUpdateWorker, InstallWorker, 
                     ...
                     SearchListWorker, DetailWorker, AurInstallerWorker, SelfUpdateWorker)
...
class RubiAUR(QMainWindow):
    def __init__(self):
        ...
        self.CURRENT_VERSION = "1.2" # La versión actual de tu app
    ...
    def check_dependencies(self):
        # Usamos tu herramienta de utils.py
        if not is_aur_helper_installed():
            self.header_frame.hide()
            self.stacked.setCurrentIndex(5) # Tu pantalla de carga
            
            # Volvemos a tu texto fijo usando los idiomas
            ...
            # Lanzamos el instalador normalmente
    ...
    def on_aur_installed(self, success, msg):
        if success:
            ...
            # ¡Aquí está la magia! Mostramos la notificación con el error de la terminal
            self.toast = ToastNotification(self.centralWidget(), "Error de Instalación", msg, self.is_dark)
            self.toast.show_anim()
            # Aún así iniciamos la app para no dejarte atrapado en la pantalla de carga
            self.finish_startup()
    ...
    def init_ui(self):
        ...
        self.settings_badge.move(26, 0) # Lo movemos a la esquina superior derecha del botón
        self.settings_badge.hide()      # Lo ocultamos por defecto
        ...
        # Le pasamos self.tr() a las descripciones y al badge
        ...
        # 1. Botón de Restablecer (Primero)
        ...
        #set_lay.addSpacing(1)
        #set_lay.addWidget(ver_lbl)
    ...
    def launch_app(self):
        ...
        # Diccionario mágico: Aquí ponemos los comandos reales de las apps que tienen nombres raros
        comandos_especiales = {
            "visual-studio-code": "code",
            "google-chrome": "google-chrome-stable",
            "intellij-idea-community-edition": "idea",
            "pycharm-community-edition": "pycharm",
            "github": "github-desktop",           # <--- ¡Agrega GitHub!
            "telegram": "telegram-desktop"
        }
        
        # Si la app está en nuestro diccionario, usamos su comando. Si no, usamos el clean_name
        comando_final = comandos_especiales.get(clean_name, clean_name)
        
        try:
            # start_new_session=True hace que la app se abra de forma independiente a RubiAUR
            subprocess.Popen([comando_final], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True)
            
            # Mostramos un aviso de éxito
            ...
            # Ahora si falla, ¡sabremos por qué!
    ...
    def show_about_dialog(self):
        ...
        # Le ponemos el logo de tu app al cuadro de diálogo
        logo_pixmap = QIcon(get_resource_path("logo.svg")).pixmap(64, 64)
        if not logo_pixmap.isNull():
            msg.setIconPixmap(logo_pixmap)
        
        # Lo estilizamos para que respete el modo oscuro/claro
        if self.is_dark:
            msg.setStyleSheet("""
    ...
    def on_self_update_result(self, has_update, latest_ver, link):
        ...
        if has_update:
            self.settings_badge.show() # ¡Mostramos el puntito rojo!
            ...
            # Cambiamos la función del botón para que ahora abra GitHub
